...
import { sendVerificationCode } from "@/lib/api-calls/auth-server";
...
const AddNumberModal = () => {
  ...
  const [phone, setPhone] = useState<string | null>(null); // Updated to use phone instead of phoneNumberId
  ...
  const onSubmit = async (data: z.infer<typeof AddPhoneValidation>) => {
    ...
    setIsLoading(true);
    try {
      const response = await sendVerificationCode(data.phone);
      setPhone(data.phone);
      setOpen(false)
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Verify Your New Phone number</DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form
