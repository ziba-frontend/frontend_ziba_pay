...
const Dropdown: React.FC<DropdownProps> = ({ title, items, onOpenChange }) => {
   ...
   return (
     <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
       <DropdownMenuTrigger
         className="cursor-pointer text-[16px] hover:text-main hover:font-semibold hover:scale-105"
       ...
       >
         ...
         {items.map((item, index) => (
           ...
           >
             {title === "Products" ? (
               <div className="flex items-start group hover:bg-[#f7f7f7] p-2 rounded-xl">
                 <Image
                   src={item.img!}
                   alt={item.label}
                   width={80}
                 ...
                 <div className="flex flex-col">
                   <h4 className="group-hover:text-main text-[16px] font-semibold">
               ...
               <Link href={item.href} className="w-full text-left hover:text-main my-1">
                 <h4>{item.label}</h4>
               </Link>
             )}
           </DropdownMenuItem>
...
const Navbar: React.FC = () => {
  ...
  return (
    <div className="fixed z-[80] top-0 left-0 w-full bg-white shadow-md shadow-[#efefef] h-20 border-b border-black md:border-none">
      <div className="containe  mx-auto p-4 flex items-center justify-between h-full">
        <div className="flex items-center w-7/12 lg:justify-between md:mx-20">
          <Link href="/" className="">
          ...
          <div className="hidden md:flex w-1/2 gap-12 ml-8">
            ...
            <Link
              ...
              className="text-[16px] hover:text-main hover:font-semibold text-start"
              target="_blank"
            >
              Developer
            </Link>
            ...
            <div className="flex flex-col items-start p-4 gap-5 text-2xl">
              ...
              {user ? (
                ...
                <div className="flex flex-col gap-6 mt-4  w-full border-t justify-center items-center pt-6">
                  <Button variant="outline" className="w-5/6 p-6">
                    {" "}
      ...
      {isDropdownOpen &&
        createPortal(
          <div className="fixed opacity-50 z-[92]"></div>,
          document.body
        )}
    </div>
  );
